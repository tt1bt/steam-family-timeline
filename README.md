...
| **入库** | 每款游戏大约何时进入本机库，按可信度分成「入库」和「≤ 上界」两类（见下） |
| **分析** | 统计数据总览、时长排行、月度趋势、活跃时段热力图、成员贡献表、成就完成度、共享锁占用排行、未走共享的游戏 |

筛选：按成员、按事件类型多选，加上游戏名 / AppID / 成员的全文搜索。
时间线与游戏库复用同一套筛选条件；成员、入库与分析视图展示聚合数据，不受筛选影响。

### 「入库」视图怎么来的（重要）

**Steam 本地没有「某游戏于 X 日加入库」这个记录。** 那是服务端状态，客户端只拿到结果。
这个视图是从本机日志找旁证反推的，所以结论分成两类，界面上用不同颜色区分：

| 标注 | 含义 | 依据 |
|---|---|---|
| 🟢 **入库** | 可以当入库时间用 | 「首次下载封面」与「库缓存文件创建时间」两个**独立**信号落在同一刻 |
| ⚪ **≤ 上界** | 只能确定**不晚于**此时已在库，真实入库可能更早 | 只剩单个信号，或两个信号互相矛盾 |

**为什么老游戏判不准。** Steam 会在版本升级、库重整时**批量重建** `librarycache` 文件，
创建时间被整体刷新。实测本机 236 款有游玩记录的游戏里，
**58 款的「最后游玩时间」早于其库缓存创建时间**，最大差 1799 天——
所以那个时间对老游戏没有意义，只能当上界。

另外「首次下载封面」的日志会滚动，本机只覆盖 2025-07-23 之后，更早的入库事件已无从考证。
界面上会显示这个覆盖范围。

工具**不允许**把上界画成日期——那等于编数据。看到 `≤` 就说明这个数字是上界，不是入库时间。
...
**1. 「某游戏哪天入库」本地查不到，只能反推。**

> **Steam 本地并不保存「某款游戏是哪一天加入家庭库」的记录。**

家庭组是服务端状态，客户端只拿到结果。工具用了两条路：

- **「入库」视图** —— 从日志旁证反推，能给可信时间的给时间，
  只能给上界的明确标 `≤`，判不准的直说判不准。方法和硬伤见上一节
- **「时间线」视图** —— 记录每一次事件本身，用的是原始日志时间戳，不是推导值
...
│   ├── library_add.py    # 反推「游戏何时进入本机库」（带可信度标注）
...
| `--browser` | 用默认浏览器开**标签页**，而不是独立窗口 |
| `--window-size 1360,900` | 独立窗口尺寸 |
| `--keep-console` | 独立窗口模式下不隐藏控制台窗口 |
| `--port N` | 换端口，默认 8765 |
| `--no-browser` | 完全不打开界面（只跑服务） |
| `--refresh` | 启动时强制重新采集 |
| `--rebuild` | 只重新生成快照后退出 |
...
默认开出来的是一个**没有地址栏、没有标签页**的独立窗口，带标题栏和自己的任务栏项；
**关掉窗口程序就退出**，控制台窗口也会自动隐藏，不会留个黑窗口杵在后面。

### 为什么不用 pywebview / Electron

实测过 pywebview：源码模式能开窗，但**打成单文件 exe 后 WebView2 窗口起不来**
（`Main window failed to start`，pythonnet + PyInstaller 的经典坑），
而且 exe 会从 8.85 MB 涨到 **14.2 MB**，破坏「零第三方依赖」这个定位 ——
这个工具的价值就在「下载一个 9 MB 的文件双击就能用」。

所以改用**系统自带浏览器跑应用模式**（`Edge/Chrome --app=URL`）：

| | 应用窗口（当前方案） | pywebview |
|---|---|---|
| exe 体积 | 8.85 MB（不变） | 14.2 MB（+60%） |
| 第三方依赖 | 无 | pywebview + pythonnet + cffi + … |
| 打包可靠性 | 高（只调用外部程序） | 低（实测冻结后窗口起不来） |
| 任务栏图标 | 浏览器的 | 自己的 |

代价是任务栏图标借用了浏览器，窗口本质上还是浏览器进程。
如果哪天这个不能接受，再考虑换方案 —— 但那时候体积和可靠性都要重新权衡。

### 独立窗口会多占约 20 MB 磁盘

应用窗口需要一个**独立的浏览器配置目录**，放在 `data/browserprofile/`，约 22 MB。
用独立目录而不是复用你平时的浏览器配置，是为了不碰你的登录态、也不和已开着的窗口互相干扰。

它和缓存一样，**删掉 `data/` 就一起清了**。如果不想让它占地方，用 `--browser` 走标签页模式。

### 找不到 Edge 会怎样

自动退回默认浏览器的标签页，功能完全不受影响（`--selftest` 里能看到检测结果）。
Windows 10/11 自带 Edge，正常不会走到这一步。

### 想回到标签页

```bat
steam-family-toolbox.exe --browser
```
